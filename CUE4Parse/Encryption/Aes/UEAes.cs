...
namespace CUE4Parse.CUE4Parse.Encryption.Aes;
...
/// </summary>
public static class UEAes
{
    private const int AES_KEYBITS = 256;
    private const int AES_BLOCKBYTES = 16;
    private static int KEYLENGTH(int keybits) => keybits / 8;
    private static int RKLENGTH(int keybits) => keybits / 8 + 28;
    private static int NROUNDS(int keybits) => keybits / 32 + 6;
    ...
    private static uint GETU32(Span<byte> plaintext) => (
            ((uint)plaintext[0] << 24) ^ 
            ((uint)plaintext[1] << 16) ^ 
            ((uint)plaintext[2] << 8) ^
            ((uint)plaintext[3] << 0)
    ...
    {
        ciphertext[0] = (byte) ((st >> 24) & 0xFF);
        ciphertext[1] = (byte)((st >> 16) & 0xFF);
        ciphertext[2] = (byte)((st >> 8) & 0xFF);
        ciphertext[3] = (byte)(st & 0xFF);
    }
    ...
    public static byte[] EMDecrypt(byte[] bytes, int beginOffset, int count, bool isIndex, IAesVfsReader reader)
    {
    ...
    public static byte[] EMDecryptByAES(byte[] bytes, int beginOffset, int count, FAesKey AesKey)
    {
        if (bytes.Length < beginOffset + count)
            throw new IndexOutOfRangeException("beginOffset + count is larger than the length of bytes");
        if (count % 16 != 0)
        ...
        var rk = new uint[RKLENGTH(AES_KEYBITS)];
        rk.Clone();
        var nrounds = rijndaelSetupDecrypt(rk.AsSpan(), key, AES_KEYBITS);

        for (var offset = 0; offset < count; offset += AES_BLOCKBYTES)
        {
            rijndaelDecrypt(rk.AsSpan(), nrounds, ciphertext.AsSpan()[offset..], plaintext.AsSpan()[offset..]);
        }
